...
def addFriend(connection):
    with connection.cursor() as cursor:
        nameFriend = multenterbox("Вкажіть контакти друга для додавання та свій логін авторизації:",'friend',['Name','Surname', 'Login'])
...
def selectAllFromUser(connection):
    with connection.cursor() as cursor:
        ...
        result = cursor.fetchall()
    return msgbox(f"{result[0]['Name']} {result[0]['Surname']}, Login: {result[0]['Login']}, Password: {result[0]['Parol']}",image='programer.gif')
...
def delPost(connection):
    with connection.cursor() as cursor:
        delPost= multenterbox('Вкажіть Назву поста,який ви хочете видалити та свій логін авторизації:', 'DELETE', ['Name', 'Login'])
